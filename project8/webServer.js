...
app.post('/admin/login', function(request, response) {
    ...
    User.findOne({login_name: login_name}, function(err, query) {
        ...
        user.name = query.first_name + " " + query.last_name;
        response.status(200).send(user);
    });
});
...
app.post('/admin/logout', function(request, response) {
    if (request.session.login_name && request.session.user_id) {
		var userObj = {};
        User.findOne({login_name: request.session.login_name}, function(err, query) {
            if (query === null) {
                response.status(400).send("login_name doesn't exist");
                return;
            }
            ...
            userObj.name = query.first_name + " " + query.last_name;
            userObj.user_id = query._id;
        });
...
app.post('/user', function(request, response) {
    ...
    User.findOne({login_name: user.login_name}, function(err, query) {
        ...
        function done_callback(err1, obj) {
            ...
            console.log("obj");
            console.log(obj);
            output.name = obj.first_name + " " + obj.last_name;
            output.user_id = obj._id;
...
app.post('/likeOrUnlike', function(request, response) {
    if (!Object.prototype.hasOwnProperty.call(request.session, "user_id")) {
        response.status(401).send("Please Login");
        return;
    }
    ...
    Photo.findOne({_id: photo_id}, function(err, query) {
        ...
        // already liked
        if(like) {
            if (!query.likes.includes(user_id)) {
                // response.status(500).send("You have liked this photo.");
                query.likes.push(user_id);
                // return;
            }
        ...
        // unlike
            ...
            query.likes.splice(query.likes.indexOf(user_id), 1);
        }

        // save query
        query.save(function(err1, res) {
            if (err1) {
                response.status(500).send(JSON.stringify(err1));
                return;
            }
...
app.post('/deleteUser/:id', function(request, response) {
    ...
    // delete photo
    Photo.deleteMany({user_id: request.params.id}, function(err) {
        if (err) {
            console.log(err);
            response.status(400).send(JSON.stringify(err));
            // return;
    ...
    // delete activity
    Activity.deleteMany({user_id: request.params.id}, function(err) {
        if(err) {
            console.log(err);
            response.status(400).send(JSON.stringify(err));
            // return;
    ...
    Photo.find({}, function(err, query) {
        ...
        for (let i = 0; i < query.length; i++) {
            // comments
            let commentList = query[i].comments;
            commentList = commentList.filter(function(comment) {
                return comment.user_id.toString() !== request.params.id;
            });
            query[i].comments = commentList;
            // likes
            let likes = query[i].likes;
            if (likes.includes(request.params.id)) {
                likes.splice(likes.indexOf(request.params.id), 1);
                query[i].likes = likes;
            }
            query[i].save();
        }
    ...
    User.deleteOne({_id: request.params.id}, function(err) {
        if(err) {
            response.status(400).send(JSON.stringty(err));
            return;
        }
